...
namespace Teachify.API.Controllers
{
    ...
    public class ProvidersController : ControllerBase
    {
        ...
        public ProvidersController(IConfiguration _configuration,
            IUsersBL _usersBL,
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        [HttpGet("getcityproviders")]
        public async Task<ActionResult<List<ProviderVM>>> GetCityProviders(string subject, string gender, string city)
        {
            var providers = await providersBL.GetProviders();
            return providers.Where(x=>x.City==city && x.Gender==gender && x.CourseDomain==subject).Select(x => new ProviderVM
            {
                City = x.City,
                CourseDomain = x.CourseDomain,
                Description = x.Description,
        ...
        public async Task<ActionResult<bool>> NewProvider(ProviderVM p)
        {
            bool result = false;
            if (ModelState.IsValid)
            {
                ...
                if (provider != null)
                {
                    ...
                    int id = await providersBL.AddProvider(provider);
                    if (id > 0)
                    {
                        result = true;
                    }
